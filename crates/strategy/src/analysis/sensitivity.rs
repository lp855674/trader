...
use crate::optimizer::grid::{ParameterSpace, GridSearchResult};
...
impl SensitivityAnalyzer {
        ...
        F: FnMut(&HashMap<String, f64>) -> f64,
    {
        ...
        for param_name in &keys {
            ...
            sensitivities.insert(param_name.clone(), ParameterSensitivity {
        ...
        let overall_robustness = if total == 0 { 1.0 } else { robust_count as f64 / total as f64 };

        let most_sensitive_param = sensitivities
            .values()
            .max_by(|a, b| a.gradient.abs().partial_cmp(&b.gradient.abs()).unwrap_or(std::cmp::Ordering::Equal))
...
fn compute_gradient(pairs: &[(f64, f64)]) -> f64 {
    ...
    if dx.abs() < 1e-10 { 0.0 } else { (last.1 - first.1) / dx }
}
...
fn pearson_correlation(results: &[GridSearchResult], a: &str, b: &str) -> f64 {
    ...
    let n = pairs.len();
    if n < 2 { return if a == b { 1.0 } else { 0.0 }; }
    ...
    let std_a: f64 = pairs.iter().map(|p| (p.0 - mean_a).powi(2)).sum::<f64>().sqrt();
    let std_b: f64 = pairs.iter().map(|p| (p.1 - mean_b).powi(2)).sum::<f64>().sqrt();
