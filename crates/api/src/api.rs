    ...
    let created = state
        ...
        .create_fee_rule_with_tiers(storage::NewFeeRuleWithTiers {
            rule: storage::NewFeeRule {
                ...
                volume_window: request.volume_window.unwrap_or_else(|| "run".to_string()),
...
fn fee_rule_with_tiers_response(rule: storage::StoredFeeRuleWithTiers) -> FeeRuleWithTiersResponse {
    FeeRuleWithTiersResponse {
        rule: FeeRuleResponse {
            ...
            volume_window: rule.rule.volume_window,
...
fn configured_market_data_provider(
...
) -> Result<Box<dyn MarketDataProvider>, ApiError> {
    match app_config.market_data.provider {
        ...
        config::MarketDataProviderKind::Longbridge => {
            ...
            Ok(Box::new(
                LongbridgeMarketDataProvider::from_env(&settings)
                    .map_err(|error| ApiError(anyhow::anyhow!(error)))?,
            ))
        }
    }
...
async fn run_realtime_market_data_gate(
    ...
    if seen.is_empty() {
        return Err(ApiError(anyhow::anyhow!(
            "paper market data gate requires at least one configured strategy symbol"
        )));
    }
