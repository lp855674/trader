...
`trader-server` 是部署级控制面，不是某一个策略或某一种运行模式的包装进程。Server 启动配置只描述数据库、监听地址、日志与默认模板位置等部署关注点。

Run 是执行单元。每个 run 通过 `RunSpec` 明确绑定：
...
portfolio / risk
data source
config snapshot or config version
```

同一个 server 可以同时管理多个 Backtest、Replay、Paper、Live run。任何订单、成交、持仓、账户、快照、指标、事件与日志读取都必须显式带 `run_id`，不能从 server 全局配置推断。
...
### 5.2 Control Plane 与 Runtime Manager

Control Plane 负责接收启动、停止、查询和控制请求，校验并持久化每个 run 的 `RunSpec` 或配置版本绑定。
