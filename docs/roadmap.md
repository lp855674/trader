...
- `scripts/ops-smoke.ps1` now provides the end-to-end local ops smoke across broker-agnostic snapshot/recovery focused gates, live run startup, snapshots, reconciliation, reconciliation alert summary, logs, config-version readback, and staging config governance readback in the same local environment.
