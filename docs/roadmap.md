...
- API-launched Backtest, Paper, Replay, and Live runs capture `RUN` config snapshots in `configs`; managed configs now support version creation, draft/pending_review/approved/published/archived state transitions, JSON diff, rollback-to-new-draft, release/audit/event logging, run-version bindings, and API/CLI status queries for local lifecycle tracking. Production permissions, multi-environment approval, and rollout policy enforcement remain follow-up work.
