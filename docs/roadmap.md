...
- `scripts/ops-smoke.ps1` now provides the end-to-end local ops smoke across broker-agnostic snapshot/recovery focused gates, market-rule runtime/governance focused gates, live run startup, snapshots, reconciliation, reconciliation alert summary, logs, config-version readback, config governance policy readback, staging approval queue readback, and production quorum governance in the same local environment.
