...
## 2026-07-12 Local Filled-Order Reconciliation Proof

- Local deterministic filled-order reconciliation is now covered with fake/test broker evidence. Runtime reconciliation maps local fills into `RuntimeExecution` with `side`, `symbol`, `account_id`, `client_order_id`, `broker_order_id`, `price`, `qty`, and `fee`, then compares those fields against broker-reported executions after identity matching.
