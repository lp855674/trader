...
- Date: 2026-07-08
- Scope: archival replay of accepted real-broker-derived IBKR paper and Binance Testnet evidence through `scripts/reconciliation/live-reconciliation-gate.ps1`
- Status: completed
- Failure class: ok
- Decision: allow
...
| --- | --- | --- |
| `ibkr:DU****91` | `docs/results/production-reconciliation/production-reconciliation-acceptance-summary.md`; `data/reconciliation/production/production-reconciliation-ibkr-83c4db22f6a9/summary.json` | 30 read-only paper-account audits, `status=completed`, `failure_class=ok`, all drift and stale counters `0` |
...
```powershell
powershell -ExecutionPolicy Bypass -Command "& .\scripts\reconciliation\live-reconciliation-gate.ps1 -Config 'data/reconciliation/live-gate-replay/real-broker-replay-2026-07-08.toml' -Account @('ibkr:DU****91','binance:binance-testnet') -MinSuccessfulAudits 3 -MaxAuditAgeMs 300000"
```
