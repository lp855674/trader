...
```text
data/verification/paper-readiness/{readiness_id}/summary.json
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\check\paper-readiness.ps1 -SkipBinance -SkipIbkr
powershell -ExecutionPolicy Bypass -File .\scripts\check\paper-readiness.ps1 -SkipCargo -SkipBinance
powershell -ExecutionPolicy Bypass -File .\scripts\check\paper-readiness.ps1 -SkipCargo -SkipReferenceData -SkipBinance -SkipIbkr
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\binance\binance-paper-smoke.ps1 -SkipNetwork
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\binance\binance-paper-run.ps1 -Limit 100 -ConfirmTestnetOrder
```
...
Known probe errors include an `action:` suffix:

- `10089`: enable the required real-time API market-data subscription; for the
  current NASDAQ stock universe, verify `NASDAQ.NMS/TOP/ALL`.
- `10168`: enable the required real-time API subscription. Delayed data may be
  used only for diagnosis and does not open the order-submission gate.
- `10197`: close or resolve the competing TWS/IB Gateway trading session for
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-soak.ps1 `
  -Iterations 3 `
  -AccountId DU12345 `
  -GatewayHost 127.0.0.1 `
  -Port 4002 `
