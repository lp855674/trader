...
- P1 存储边界：Backtest、Paper、API、CLI 生产路径不再直接构造 storage 写入 DTO；运行层改走 `storage` 暴露的语义 command / repository 方法。`scripts/check/check-storage-dto-boundary.ps1` 已扩大到 `crates` 与 `apps`，用于防止 storage DTO 写入形态再次泄漏到边界之外。
...
- 后续切片 API read model 隔离：REST 查询路由已把 run/event storage record 映射成 API-owned `RunResponse` / `EventResponse`，不再把 storage record 类型作为 HTTP response contract；`scripts/check/check-api-read-model-boundary.ps1` 已接入 `scripts/check/verify.ps1`。
...
- 后续切片边界门禁跨平台收口：DB 边界、storage DTO 边界、API read model 边界均已补齐 bash 版本，并接入 Linux/macOS `scripts/check/verify`；Windows `verify.ps1` 与 bash `verify` 执行同一类架构门禁。
...
- `powershell -ExecutionPolicy Bypass -File .\scripts\check\verify.ps1`（默认 `Jobs=1`，包含 `cargo fmt --all -- --check`、`cargo check --workspace -j 1`、`cargo test --workspace -j 1`）
...
- `powershell -ExecutionPolicy Bypass -File .\scripts\smoke\mvp-smoke.ps1`
- `powershell -ExecutionPolicy Bypass -File .\scripts\smoke\paper-smoke.ps1`
- `git diff --check`
...
当前状态：Backtest、Paper、API、CLI 生产写入路径已改为调用 `storage` 暴露的语义 command / repository 方法；边界外不再直接构造 storage 写入 DTO。`scripts/check/check-storage-dto-boundary.ps1` 已作为回归检查。
...
11. 每个切片跑对应验证；关键切片至少跑 `scripts/smoke/mvp-smoke.ps1`、`scripts/smoke/paper-smoke.ps1`，并按触及范围补充 Binance/IBKR 无网络 smoke。
