...
Binance testnet adapter 已开始接入。当前支持 `ping`、signed account snapshot，以及手动 tiny limit order -> query -> cancel -> myTrades sync -> local accounting snapshot；不支持策略自动提交真实 testnet 订单，也不支持完整 OMS recovery 或 broker account/position reconciliation。凭证只从环境变量读取：
