...
Binance testnet adapter 已开始接入。当前支持 `ping`、signed account snapshot，以及手动 tiny limit order -> query -> cancel -> myTrades sync -> local accounting snapshot；也已提供受闸门保护的策略自动 Binance Spot Testnet executor。`[broker] order_submit_enabled` 是策略自动送单闸门，默认 `false`；打开后 `paper-run` 只在 `broker.kind = "binance"`、`broker.mode = "paper"`、Spot Testnet `base_url` 和环境变量凭证都满足时才会提交 testnet limit order。自动 Binance paper run 启动时会读取 Binance account snapshot，并用 USDT cash 覆盖本次 `PaperSettings.initial_cash`。自动送单使用稳定的 `client_order_id = trader-paper-{run_id_prefix}-{order_number}`，broker adapter 已支持通过 Binance `origClientOrderId` 构造恢复查询。该 executor 只把 Binance `myTrades` 返回的真实成交写入本地 fill；如果订单未成交或没有 trades，run 会失败，不会伪造成交。凭证只从环境变量读取：
