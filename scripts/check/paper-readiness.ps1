...
$readinessDir = Join-Path $repoRoot "data/verification/paper-readiness/$readinessId"
...
try {
    ...
    if (-not $SkipIbkr) {
        Invoke-Step "ibkr paper local dry-run" {
            powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-run.ps1 -SkipRefresh
        ...
        Invoke-Step "ibkr paper soak dry-run" {
            powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-soak.ps1 -Iterations $IbkrSoakIterations -SkipRefresh
