...
$verificationDir = Join-Path $repoRoot "data/verification/live-recovery/$verificationId"
...
try {
    ...
    if (-not $failed -and $IncludeBinanceReadOnly) {
        ...
        $commandArgs = @(
            ...
            "Bypass",
            "-File",
            ".\scripts\binance\binance-paper-recover-smoke.ps1"
        )
    ...
    if (-not $failed -and $IncludeIbkrReadOnly) {
        ...
        $commandArgs = @(
            ...
            "Bypass",
            "-File",
            ".\scripts\ibkr\ibkr-paper-test-guide.ps1",
            "-Stage",
            "ReadOnly",
            "-AccountId",
            $IbkrAccountId,
