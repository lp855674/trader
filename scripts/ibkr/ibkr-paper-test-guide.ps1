...
function New-TestConfig {
    ...
    $text = $text `
        ...
        -replace 'url = "sqlite://data/ibkr/databases/ibkr-aapl-1d-paper.sqlite"', "url = `"$databaseUrl`"" `
...
function Write-TestPlan {
    ...
    Write-Host "powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-test-guide.ps1 -Stage DryRun"
    Write-Host ""
    Write-Host "Read-only Gateway validation:"
    Write-Host "powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-test-guide.ps1 -Stage ReadOnly -AccountId DU..."
    Write-Host ""
    Write-Host "Manual tiny paper order. This submits a real IBKR paper LMT order:"
    Write-Host "powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-test-guide.ps1 -Stage TinyOrder -AccountId DU... -ConfirmTinyOrder"
    Write-Host ""
    Write-Host "Automatic paper-run order path. This enables order_submit_enabled only in a generated run config:"
    Write-Host "powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-test-guide.ps1 -Stage AutoRun -AccountId DU... -ConfirmAutoRun"
    Write-Host ""
    Write-Host "Multi-iteration soak. Default is local-only; add AccountId and confirmation after Gateway is ready:"
    Write-Host "powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-soak.ps1 -Iterations 3 -SkipRefresh"
    Write-Host "powershell -ExecutionPolicy Bypass -File .\scripts\ibkr\ibkr-paper-soak.ps1 -Iterations 3 -AccountId DU... -ConfirmIbkrPaperOrder"
...
function Invoke-DryRun {
    ...
    $args = @("-ExecutionPolicy", "Bypass", "-File", ".\scripts\ibkr\ibkr-paper-run.ps1")
    if ($SkipRefresh) {
        $args += "-SkipRefresh"
    }
    powershell @args
...
function Invoke-AutoRun {
    ...
    $args = @(
        ...
        "Bypass",
        "-File",
        ".\scripts\ibkr\ibkr-paper-run.ps1",
        "-AccountId",
        $AccountId,
        "-GatewayHost",
        $GatewayHost,
