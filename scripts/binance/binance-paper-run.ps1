...
try {
    ...
    if (-not $SkipRefresh) {
        powershell -ExecutionPolicy Bypass -File .\scripts\binance\binance-refresh-klines.ps1 `
            -Config $Config `
            -Symbol $Symbol `
            -Interval $Interval `
            -Limit $Limit
    ...
    $configText = $configText `
        ...
        -replace 'url = "sqlite://data/binance/databases/binance-btcusdt-1m-paper.sqlite"', "url = `"$databaseUrl`""
