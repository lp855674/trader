...
try {
    ...
    $cryptoMarketMeta = Invoke-WebRequest -UseBasicParsing "$baseUrl/api/v1/crypto-market-meta?exchange=BINANCE&symbol=BTCUSDT_PERP"
    $corporateActions = Invoke-WebRequest -UseBasicParsing "$baseUrl/api/v1/corporate-actions?market=US&symbol=US:NASDAQ:AAPL:EQUITY&start_ms=0&end_ms=1"
    ...
    Assert-True (@($configs).Count -ge 1) "expected config snapshots"
    Assert-True (@($insights).Count -ge 1) "expected strategy insights"
    Assert-True (@($targets).Count -ge 1) "expected portfolio targets"
