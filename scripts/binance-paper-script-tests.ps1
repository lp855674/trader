...
Write-FakeTrader "ok"
$successOutput = powershell -ExecutionPolicy Bypass -File .\scripts\binance-paper-run.ps1 -SkipRefresh 2>&1
...
try {
    $haltOutput = powershell -ExecutionPolicy Bypass -File .\scripts\binance-paper-run.ps1 -SkipRefresh 2>&1
} finally {
...
$ErrorActionPreference = "Continue"
try {
    $openOrdersOutput = powershell -ExecutionPolicy Bypass -File .\scripts\binance-paper-run.ps1 -SkipRefresh 2>&1
} finally {
    $ErrorActionPreference = $previousErrorActionPreference
}
